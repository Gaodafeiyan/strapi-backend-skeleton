...
async function testAuthAPIs() {
  ...
  const registerResult = await testEndpoint('POST', '/api/auth/local/register', registerData, '用户注册（使用邀请码）');
...
// 3. 测试邀请码相关API
async function testInviteCodeAPIs() {
  console.log(colors.cyan('\n🎯 测试邀请码相关API...'));
...
// 13. 测试邀请码统计和排行榜
async function testInviteStatistics() {
  console.log(colors.cyan('\n📊 测试邀请码统计和排行榜...'));
...
async function runAllTests() {
  console.log(colors.blue('🚀 开始全面测试后端API（包含邀请码DEF678901）...\n'));
  
  try {
    await testBasicAPIs();
    await testAuthAPIs();
    await testInviteCodeAPIs();
