...
export default factories.createCoreController('api::choujiang-jihui.choujiang-jihui', ({ strapi }) => ({
      ...
      for (const chance of chances) {
        ...
        try {
          ...
          for (let i = 0; i < count; i++) {
            ...
            createdChances.push(chanceRecord);
          }

          results.push({
            userId,
            success: true,
            chances: createdChances,
            message: '赠送成功'
          });
  ...
  async useChance(ctx) {
    try {
      ...
      if (chance.yonghu !== userId) {
        return ctx.forbidden('无权使用此抽奖机会');
      }
      
      if (chance.used) {
        return ctx.badRequest('此抽奖机会已使用');
      }
      
      if (chance.status !== 'available') {
        return ctx.badRequest('此抽奖机会不可用');
      }
