  ...
  ({ strapi }) => ({
    ...
    async createOrder(userId: number, jihuaId: number) {
      ...
      return await executeWithLock(lockKey, async () => {
        // 检查用户操作频率限制
        if (!globalRateLimiter.checkRateLimit(userId.toString(), 'createOrder', 5, 60000)) {
    ...
    async redeem(orderId: number, options: { force?: boolean; testMode?: boolean } = {}) {
      ...
      return await executeWithLock(lockKey, async () => {
        ...
        if (order.zhuangtai === 'active' && isOrderExpired && !force && !testMode) {
            ...
            async (trx) => {
              ...
              await trx.query('api::dinggou-dingdan.dinggou-dingdan').update({
                ...
                data: {
                  ...
                  aiShouYi: aiQty,
                  aiTokenShouYi: aiTokenAmount,
                  aiTokenId: selectedToken?.id || null,
                  aiTokenJiaGe: tokenPrice,
                  version: (order.version || 0) + 1
                }
              });

              // ④ 处理邀请奖励
              if (order.yonghu.shangji) {
                try {
                  await strapi.service('api::yaoqing-jiangli.yaoqing-jiangli').processInvitationReward(
                    order.yonghu.shangji.id,
                    order.yonghu.id,
                    staticUSDT,
                    aiQty
                  );
                } catch (rewardError) {
                  console.error('邀请奖励处理失败:', rewardError);
                  // 邀请奖励失败不影响主流程
                }
              }
